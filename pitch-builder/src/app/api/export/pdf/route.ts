...
import { exportProjectToPDF } from '@/lib/export/pdf-exporter'
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate PDF
    const pdfBlob = await exportProjectToPDF(projectData)
    ...
    await prisma.exportJob.create({
      data: {
        ...
        format: 'PDF',
        status: 'completed',
        fileUrl: `exports/${projectId}_${Date.now()}.pdf`
      }
    })
    ...
    return new NextResponse(buffer, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="${project.title || 'project'}.pdf"`,
